...
- **Создание**: `build_prefab` разворачивает компактный спек (~30 строк) в полный корректный `.prefab` + `.meta` (~400–800 токенов вместо 15–50k при ручной генерации JSON); `build_animgraph` так же собирает анимационные графы (`.animgraph`).
