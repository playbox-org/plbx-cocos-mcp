...
| `inspect_node` | Полное поддерево конкретного нода без фильтрации |
...
| `lint_assets` | Криптоимена (`mesh_001`, `Sprite(2)`), разброс размеров моделей, нарушения wrapper-правила |
...
**Политика префабов**: правь исходный ассет, не инстанс — инстансы наследуют изменения. Внутренности инстансов в файле не существуют (свёрнутая сериализация) и правятся только через overrides.
