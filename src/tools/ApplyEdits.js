...
- insert_array_element {node, component?, componentIndex?, property, index?, from?, type?, value?} (adds ONE element to an array-valued property such as a CardEntry[] — plain set_component_property only edits the fields of EXISTING elements. index: insert position (append if omitted). For an array of references to typed objects, a neighbor element (from = its index, default 0) is deep-cloned as the skeleton — that is where the full field set comes from, a user CCClass has no default registry — and value {..} is merged over its top-level fields; nested reference fields (config, members) are edited afterwards with set_component_property on the new index. type is validated against the neighbor (required only for an empty array). Inline value-type arrays (cc.Vec2[]) and scalar arrays are supported too.)
- remove_array_element {node, component?, componentIndex?, property, index, force?} (removes ONE element from an array-valued property. A referenced element's owned objects are garbage-collected on save; reachable references into it from elsewhere block the removal unless force nulls them — same rule as remove_node/remove_component)
